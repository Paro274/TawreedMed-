...
#final_price, #package_price_display {
    ...
    background: #f8f9fa; font-weight: 800;
}
#discount-field {
...
<select name="status">
<option value="مقبول" {{ $product->status == 'مقبول' ? 'selected' : '' }}>مقبول</option>
<option value="معلق" {{ $product->status == 'معلق' ? 'selected' : '' }}>معلق</option>
<option value="مرفوض" {{ $product->status == 'مرفوض' ? 'selected' : '' }}>مرفوض</option>
</select>
...
function toggleFieldsByProductType() {
    ...
    if (productType === 'أدوية') {
        ...
        // الحفاظ على القيمة المختارة عند التغيير لو أمكن
        const selectedValue = packageTypeSelect.dataset.selected || 'كرتونة';
        
        packageTypeSelect.innerHTML = `
            <option value="كرتونة" ${selectedValue == 'كرتونة' ? 'selected' : ''}>كرتونة</option>
            <option value="كيس" ${selectedValue == 'كيس' ? 'selected' : ''}>كيس</option>
            <option value="علبة" ${selectedValue == 'علبة' ? 'selected' : ''}>علبة</option>
        `;
    } else {
        ...
        const selectedValue = packageTypeSelect.dataset.selected || 'علبة';
        
        packageTypeSelect.innerHTML = `
            <option value="علبة" ${selectedValue == 'علبة' ? 'selected' : ''}>علبة</option>
            <option value="كيس" ${selectedValue == 'كيس' ? 'selected' : ''}>كيس</option>
            <option value="دستة" ${selectedValue == 'دستة' ? 'selected' : ''}>دستة</option>
            <option value="كرتونة" ${selectedValue == 'كرتونة' ? 'selected' : ''}>كرتونة</option>
        `;
    }
