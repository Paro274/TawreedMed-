...
@section('title', $product->name . ' - مستحضرات التجميل - توريد ميد')

@push('styles')
<link rel="stylesheet" href="{{ asset('frontend/css/product-details.css') }}">
<style>
/* ✅ تحسينات CSS لمستحضرات التجميل - نفس ستايل المستلزمات */
...
/* ✅ إخفاء التقييمات والمكونات للتوحيد مع المستلزمات */
.no-reviews, .reviews-section, .ingredients-section {
...
<section class="breadcrumb-section">
    <div class="container">
        <div class="breadcrumb">
            ...
            <span>/</span>
            <a href="{{ route('frontend.cosmetics.index') }}">مستحضرات التجميل</a>
            <span>/</span>
    ...
    <div class="container">
        <div class="product-main row">
            ...
            <div class="product-gallery col-lg-5">
                ...
                @else
                <div class="no-image">
                    <i class="fas fa-magic fa-5x text-muted"></i>
                </div>
                @endif
            ...
            <div class="product-info-main col-lg-7">
                ...
                <div class="product-company mt-2">
                    <span class="label"><i class="fas fa-industry"></i> العلامة التجارية:</span>
                ...
                <div class="product-category mt-2">
                    ...
                    <a href="{{ route('frontend.cosmetics.index', ['category' => $product->category->id]) }}">
        ...
        <!-- المواصفات الأساسية -->
        @if($product->size || $product->weight || $product->skin_type || $product->suitable_for)
        <div class="product-specifications mt-5">
            ...
            <div class="row">
                ...
                @if($product->weight)
                <div class="col-md-6 mb-2">
                    <strong>الوزن:</strong> {{ $product->weight }} جم
                </div>
                @endif
                @if($product->skin_type)
                <div class="col-md-6 mb-2">
                    <strong>نوع البشرة:</strong> {{ $product->skin_type }}
                </div>
                @endif
                @if($product->suitable_for)
                <div class="col-md-6 mb-2">
                    <strong>مناسب لـ:</strong> {{ $product->suitable_for }}
                </div>
                @endif
                @if($product->units_per_package)
                <div class="col-md-6 mb-2">
...
let productPrice = parseFloat({{ $product->final_price ?? $product->price ?? 0 }});

// Debug: Log initial values
console.log('Initial Product Price:', productPrice);
console.log('Initial Quantity:', currentQuantity);

// Test calculation immediately
console.log('Test: 100 × 8 =', productPrice * 8);
...
function updateTotalPrice() {
    ...
    // Format and update
    const formattedPrice = totalPrice.toFixed(2) + ' جنيه';
    totalPriceSpan.textContent = formattedPrice;
    if (cartButtonPrice) {
        cartButtonPrice.textContent = '(' + formattedPrice + ')';
    }
    
    console.log('Updated display to:', formattedPrice);
    console.log('=== END ===');
}
...
function addToCart(productId) {
    ...
    // التحقق من التوفر
    const maxStock = {{ is_numeric($product->available_quantity) ? $product->available_quantity : '999' }};
    if (quantity > maxStock && maxStock !== 999) {
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                icon: 'warning',
                title: 'كمية غير متوفرة',
                text: `الكمية المطلوبة (${quantity} ${unitType}) غير متوفرة في المخزون! المتوفر: ${maxStock} ${unitType}`
            });
        } else {
            alert(`الكمية المطلوبة (${quantity} ${unitType}) غير متوفرة في المخزون! المتوفر: ${maxStock} ${unitType}`);
        }
        return;
    }
...
function notifyWhenAvailable(productId) {
    ...
    if (email && email.includes('@')) {
        alert('✅ تم تسجيل بريدك الإلكتروني! سنخطرك عند توفر المنتج.');
